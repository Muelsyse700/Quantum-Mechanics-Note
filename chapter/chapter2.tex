...
\section{动量与空间平移算符}
把$x'$处的态$\psi(x')$平移到$x'+a$处：
\begin{align*}
    \psi(x'+a) &= \psi(x') + \frac{a}{1!}\dv{x'}\psi(x') + \frac{a^2}{2!} \dv[2]{}{{x'}} \psi(x') + \cdots \quad \text{Taylor展开} \\
    \intertext{若$a$极小，取$1$阶近似：}
    &\approx \psi(x') + \frac{a}{1!}\dv{x'} \psi(x') \\
    &= \left(1+a\dv{x'}\right) \psi(x')
\end{align*}
令$\hat{g}(a) \equiv 1+a\dv{x'}$，称之为无穷小平移算符.

分析力学表明，动量是无穷小平移生成元.

\begin{derivation}
    尝试基于$\hat{g}(a)$中的$\dv{x}$构造动量算符$\hat{p}$：

    $\hat{g}(a)$中的求导算符$\dv{x'}$是反厄米算符，乘以$\pm i$使之变成厄米的.

    $\pm i\dv{x'}$的量纲为$L^{-1}$，$\hat{p}$的量纲为$MLT^{-1}$，乘以量纲为$ML^2T^{-1}$(角动量量纲)的$\hbar$，则：
    \begin{align*}
        \hat{g}(a) &= 1 \pm \frac{1}{\hbar}\cdot\left(\pm ai\hbar\dv{x}\right) \\
        &= 1 \mp \frac{i}{\hbar}\cdot\left(\pm ai\hbar\dv{x}\right) \\
        &= 1 + \frac{ia}{\hbar}\cdot\left(-i\hbar\dv{x}\right)
    \end{align*}
\end{derivation}
...
\begin{derivation}
    尝试构造有限大平移算符：
    \begin{align*}
        \psi(x'+a) &= \psi(x') + \frac{a}{1!}\dv{x'}\psi(x') + \frac{a}{2!}\dv[2]{}{{x'}}\psi(x') + \cdots \\
        &= \left( 1 + \frac{\frac{i}{\hbar}\hat{p}a}{1!} + \frac{\frac{i}{\hbar}\hat{p}a}{2!} + \cdots \right) \psi(x') \\
        &= e^{\frac{i}{\hbar}\hat{p}a}\psi(x')
    \end{align*}
    令$\hat{T}(a) \equiv e^{\frac{i}{\hbar}\hat{p}a}$，称之为\overtext{空间平移算符}{spatial translation operator}.
\end{derivation}

$\hat{T}$对本征矢的作用：
\begin{itemize}
    \item $\begin{cases}
        (\bra{x'}\hat{T}(a))^\dagger = \hat{T}^\dagger(a)\ket{x'} = \ket{x'+a} \\
        \hat{T}(a)\ket{x'} = \ket{x'-a}
    \end{cases} \Rightarrow \quad \hat{T}(-a) = \hat{T}^\dagger(a)$ 对本征基的移动
    \item $1+\frac{i\hat{p}}{\hbar}a\ket{x'} = \ket{x'-a}$
    \item $1-\frac{i\hat{p}}{\hbar}a\ket{x'} = \ket{x'+a}$
\end{itemize}